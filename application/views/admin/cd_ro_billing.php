			...
			<div class="block">
				...
				<div class="block-title">
					<h2>
						...
						<strong>Classified Display Ro Billing</strong>
					</h2>
				</div>
...
function hide_div()
{
	...
	var c=s_days.length;
		...
		//	erate= parseFloat($("#brate"+i).val());
			
			o_rate= parseFloat($("#rate"+(i-1)).val());
		//	o_erate= parseFloat($("#brate"+(i-1)).val());
...
function save_ro()
{
			...
			$.each(arr, function(i, d) {
				...
				{	
					...
					return;
						...
						var  eprice= parseFloat($("#brate"+id).val());
	...
	if(pack=="")
	{
		...
		for(i=0;i<inse;i++)
		{
			...
			eprice= eprice + parseFloat($("#brate"+i).val());				
		}
		price= price/inse;
		eprice= eprice/inse;
  ...
  function get_state()
        {
            ...
            $.ajax({                
                url: "<?php echo base_url(); ?>" + "admin/display_ro/get_state",
                type: "POST",				
                async: true ,               
...
function add_paper()
{
	...
	var q = document.getElementById('pub_dates');
    if ($('#add_on').is(":checked")) 
	{
        x.style.display = 'block';
...
//    alert(ro_id+work_year+ro_no+paper_id+"tt"+p_date+"tt"+rate+erate+insertion);
	$.ajax({  
...
function update_discount()
{
	...
	type: "POST",
	async: true ,
	data: {client_id:client_id,type_id:type_id,cat_id:cat_id,newspaper_id:newspaper_id,discount_percentage:discount_percentage},
	beforeSend: function(){ document.getElementById("loader").style.display = "block";},
	success: function(data)
		{
			document.getElementById("loader").style.display = "none";
