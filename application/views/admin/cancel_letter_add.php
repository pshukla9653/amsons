				...
				<div class="form-group">
                        ...
                        <div class="col-md-9">
							<select id="letter_type" name="letter_type"  class="form-control" data-placeholder="Choose Classes" required>
								...
								<option value="C" >Cancel Letter</option>
								<option value="Pre" >Prepone Letter</option>
								<option value="Post" >Postpone Letter</option>
                            </select>
                        </div>
                    </div>	
					<div class="form-group">
	...
	$.ajax({                
			...
			success: function(data)
				...
				// 	p_u.innerHTML="";
					...
					var count=1;
					var c=0;
					$.each(data.dops1, function(i, d) 
					{	var today=new Date();
					var dop=new Date(d.dop);
					    
					    if(dop >= today)
					    {
					        if($("#letter_type").val()=="C")
					        {
						$("#ecom-products-cancel tbody").append('<tr><td><input type="checkbox" id="ro_main_id" name="ro_main_id['+c+']" value="'+d.id+'"><input type="hidden" id="paper_id" name="paper_id['+c+']" value="'+d.paper_id+'"></td><td class="text-center"><strong>'+d.ro_no+'</strong></td><td><input type="text" id="dop" name="cdop['+c+']" value="'+d.dop+'" readonly></td><td class="text-center"><input type="text" id="inse_no" name="inse_no['+c+']" value="'+count+'th" readonly></td><td class="text-center"><input type="text" id="new_dop" name="new_dop['+c+']" value="" readonly></td></tr>');
					        }
					        else
					        {
					            	$("#ecom-products-cancel tbody").append('<tr><td><input type="checkbox" id="ro_main_id" name="ro_main_id['+c+']" value="'+d.id+'"><input type="hidden" id="paper_id" name="paper_id['+c+']" value="'+d.paper_id+'"></td><td class="text-center"><strong>'+d.ro_no+'</strong></td><td><input type="text" id="dop" name="cdop['+c+']" value="'+d.dop+'" readonly></td><td class="text-center"><input type="text" id="inse_no" name="inse_no['+c+']" value="'+count+'th" readonly></td><td class="text-center"><input type="date" id="new_dop" name="new_dop['+c+']" value="" ></td></tr>');
					        }
						c++;
					    }
					    count++;
...
var add_dop = [];
// var val=$('input[name="ro_main_id[]"]').length;
// var dop=[];
// var ins_no=[];
// var ro_main_id=[];
// $('input[name="ro_main_id[]"]').each(function(){
//     if($(this).is(":checked"))
//     {
//       var id=$(this).val();
//       var dop=$("input[name='cdop[]']").val();
//       var insertion_no=$("input[name='inse_no[]']").val();
//       var data = { id : id, dop  : dop, insertion_no : insertion_no};
//       add_dop.push(data);
//     }
// });
...
{ 
   ...
   var paper_id=$("input[name='paper_id["+i+"]']").val();
       ...
       var data = { id : paper_id, ro_main_id : ro_main_id,dop  : dop, insertion_no : insertion_no, new_dop : new_dop};
       add_dop.push(data);
   }
...
// 	//var  box_c= $("#box_c").val();
 	...
 	var form_data= {'ro_no':ro_no,'ro_id':ro_id,'ro_date':ro_date,'client':client,'work_year':work_year,'sheading':sheading,'inse':inse,'scheme':scheme,'material':material,'remark':remark,'p_date':p_date,'letter_type':letter_type};
	console.log(form_data);
	$.ajax({                
			url: "<?php echo base_url(); ?>" + "admin/cancel_letter/add",
			type: "POST",				
			async: true ,               
			data: form_data,
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
