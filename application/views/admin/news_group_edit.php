	...
	<div class="row">
		<div class="col-lg-12">
			...
			<div class="block">
				...
				<form class='form-horizontal form-bordered' id='add-newspaper'
					action='<?= base_url() ?>admin/newspaper/group_edit' method="post">
					<?php
					...
					<div class="form-group">
						...
						<table id="myTable" class=" table order-list">
							...
							<?php foreach($newspaper_group_details as $row){
							?>
							<tr>
								<td class="col-lg-2">
							...
							<input type="text" placeholder="Enter Address" class="form-control" name="address[]" value="<?= $row->address ?>">
								...
								<?php foreach($states as $state){ ?>
										<option value="<?php echo $state->id; ?>" <?php echo set_select('state', $state->name); 
										if($state->id==$row->state){ echo "selected"; }
								...
								<?php 
										foreach($cities[$row->state] as $row1){ 
									?><option value="<?= $row1->id ?>" <?php if($row1->id==$row->city){ echo "selected"; } ?> ><?= $row1->name ?></option>
									<?php
								}
							?>
								</select>
								...
								<td class="col-lg-2">
									<input type="text"  placeholder="GST No." name="gst[]"  class="form-control" value="<?= $row->gst_no; ?>"/>
								</td>
								<td class="col-lg-1">
									<input type="text"  placeholder="Contact No." name="contact[]"  class="form-control"  value="<?= $row->phone; ?>"/>
								</td>
								<td class="col-lg-1">
								</td>
							</tr>
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							<input type="text" placeholder="Enter Contact Person Name.." class="form-control" name="c_p" id="c_p" value="<?php echo $newspaper_group->ng_contact_person; ?>">
						</div>
					</div>
