<div id="page-content" style="min-height: 1189px;">
	...
	<?php 
					...
					echo form_open_multipart('admin/booking_cd_ro/add', $attributes); ?>
					<?php
			...
			<div class="block">
				...
				<!--	<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							<input type="text" pattern=".*\d"  onchange="date_pic1();" min="1" placeholder="" class="form-control" name="inse" id="inse" required>
						</div>
					</div>					
					<!--
					<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-md-6">
									<div class="box"  id="from--input">
										<input class="form-control mydata" name="p_date" autocomplete="off" onblur="get_dop_price();" type="text" id="from-input" data-id="" data-toggle="modal" data-target="#dopModal" placeholder="mm/dd/yyyy, ...">
									</div>
			...
			<div class="block">
				...
				<!-- General Data Content -->
					<div class="form-group">
                        ...
                        <div class="col-md-9">
							<select id="client" name="client" style="width:100%"  onchange="get_client();" class="js-example-basic-single" data-placeholder="Choose Classes" required>
					...
					<div class="form-group">
						...
						<div class="col-md-2">
							<input type="text" placeholder="" onchange="get_size();" class="form-control" name="size_r" id="size_r" required>
						</div>
						<div class="col-md-4">
...
<div class="block full">
		...
		<div class="block-title">
			...
			<a href="<?php echo base_url()?>admin/booking_cd_ro"><div class="btn btn-sm btn-primary" style="float: right; margin-right: 10px; margin-top: 5px;">
   ...
   <script type="text/javascript">
    ...
    $(function () {
        ...
        $(".mydata").click(function () {
           ...
           // $("#name_dop").datepicker('setdate',new Date());
...
*/	
	...
	$.ajax({
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_price",
			type: "POST",				
			async: true ,               
			data: form_data,
...
//alert(id);
	...
	$.ajax({
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_fdays",
			type: "POST",				
			async: true ,               
			data: form_data,
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
...
*/	
	...
	for(i=0; i<c ;i++)
	{
		...
		//alert(s_days[i]);
		$.ajax({
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_dop_price",
			type: "POST",				
			async: true ,               
			data: form_data,
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
  ...
  function save_ro()
    {
      //  check_session_date_range();
       ...
       // var state_id = $("#state").val();
        ...
        var form_data= {'client':client,'newspaper':newspaper,'a_newspaper':a_newspaper,'state_id':state_id,'pub_id':pub_id,'title':title,'matter':matter,'cat':cat,'sheading':sheading,'pack':pack,'inse':inse,'scheme':scheme,'material':material,'party':party,'premimum':premimum,'box':box,'remark':remark,'price':price,'eprice':eprice,'w_count':w_count,'unit':unit,'size_type':size_type,'p_date':p_date,'ro_type':ro_type,'t_amount':t_amount,'p_amount':p_amount,'dis':dis,'dis_a':dis_a,'color':color,'comm1':comm1,'comm2':comm2,'comm3':comm3,'comm4':comm4,'comm5':comm5,'comm6':comm6,'comm7':comm7,'comm8':comm8,'box_c':box_c,'add_a':add_a,'odf':odf,'dop_inse':inse_dop,'free_days':free_days,'book_id':book_id};

        $.ajax({                
            url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/save",
            type: "POST",				
            async: true ,               
            data: form_data,
            beforeSend: function(){ document.getElementById("loader").style.display = "block";},
...
function get_heading()
{
	var newspaper = $("#newspaper").val();
	$.ajax({                
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_heading",
			type: "POST",				
			async: true ,               
			data: {id:newspaper},
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
...
function get_sub_heading()
{
	...
	var cat = $("#cat").val();
	$.ajax({                
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_sub_heading",
			type: "POST",				
			async: true ,               
			data: {cat_id:cat},
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
...
function get_scheme()
{
	...
	var newspaper = $("#newspaper").val();
	$.ajax({                
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_scheme",
			type: "POST",				
			async: true ,               
...
function get_scheme_price()
{
	...
	$.ajax({                
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_scheme_price",
			type: "POST",
			async: true ,
			data: {s_id:scheme_id},
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
...
function get_premimum()
{
	var newspaper = $("#newspaper").val();
	$.ajax({                
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_premimum",
			type: "POST",				
			async: true ,               
			data: {n_id:newspaper},
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
...
function get_premimum_price()
{
	...
	$.each(pre_id, function(i, id) {
		...
		$.ajax({                
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_premimum_price",
			type: "POST",				
			async: false ,               
			data: {pre_id:id},			
			success: function(data)
...
function get_package()
{
	...
	$.ajax({                
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_package",
			type: "POST",
			async: true ,
...
{		
	...
	$.ajax({
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_package_price",
			type: "POST",
			async: true ,
			data: {pack_id:pack},
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
...
else
{
      if($('#from-input').length)
      {
        ...
        if(new Date(first_date)>=new Date(second_date)){
           if(document.getElementById('state').value=="6"){
			   <?php if($cgst){ ?>
                document.getElementById('cgst').value=(parseFloat(document.getElementById('<?php echo $cgst->tax_depned;?>').value)*(<?php echo $cgst->tax_rate;?>)/2))/100;
			   ...
			   <?php }?>
                ...
                document.getElementById('sgst').value=(parseFloat(document.getElementById('<?php echo $sgst->tax_depned;?>').value)*(<?php echo $cgst->tax_rate;?>)/2))/100;
                <?php } else  {?>
                ...
                document.getElementById('sgst').value=0;
				<?php if($igst){ ?>
				document.getElementById('igst').value=(parseFloat(document.getElementById('<?php echo $igst->tax_depned;?>').value)*parseFloat(<?php echo $igst->tax_rate;?>))/100;
...
function get_paper()
{
	...
	$.ajax({                
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_newspaper",
			type: "POST",				
			async: true ,               
			data: {'paper':paper},
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
...
document.getElementById("add_pub_dates").style.display = "block";
      ...
      $.each(ar, function(i, d) {
		...
		$.ajax({
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_ad_on_price1",
			type: "POST",				
			async: true ,               
			data: form_data,			
			success: function(data)
...
*/	
	...
	$.ajax({
			url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_base_price",
			type: "POST",				
			async: true ,               
			data: form_data,
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
			success: function(data)
				...
				//	var html='<div class="title" style="background-color:#fff;"><i class="fa fa-money"></i><strong> DOP and Price of  '+ data['rates'].newspaper_name +' ,'+ data['rates'].city_name +'</strong></div>                         <div class="form-group">	<label for="pack-price" class="col-md-6 control-label" onclick="date_pic_byid('+data['rates'].newspaper_id+');">Publish Dates of '+ data['rates'].newspaper_name +' ,'+ data['rates'].city_name +'</label> <div class="col-md-6">	<div class="box"  id="from--input">	<input class="form-control" name="p_date'+ data['rates'].newspaper_id +'" type="text" id="from-input'+ data['rates'].newspaper_id +'" data-id="'+ data['values'].newspaper_id +'" data-toggle="modal" data-target="#dopModal" onclick="showModal('+data['rates'].newspaper_id+')" placeholder="mm/dd/yyyy, ..." >	</div>	<div class="code-box">	</div>	</div>	</div>                                                                       <div class="form-group"><label for="pack-price" class="col-md-3 control-label">Rate</label><div class="col-md-3"><input type="text" placeholder="" onchange="amount_calculate();" class="form-control" name="rate'+ data['rates'].newspaper_id +'" id="rate'+ data['rates'].newspaper_id +'" value="'+ data['rates'].ad_price +'"></div><label for="pack-price" class="col-md-3 control-label">B Rate</label><div class="col-md-3"><input type="text" placeholder="" onchange="amount_calculate();" class="form-control" name="brate'+ data['rates'].newspaper_id +'" id="brate'+ data['rates'].newspaper_id +'" value="0"></div></div> ';
					...
					/* <div class="form-group"><label for="pack-price" class="col-md-3 control-label">B Rate</label><div class="col-md-3"><input type="text" placeholder="" class="form-control" name="brate'+ data['values'].newspaper_id +'" id="brate'+ data['values'].newspaper_id +'"></div></div> */
						var html='<div class="title" style="background-color:#fff;"><i class="fa fa-money"></i><strong> DOP and Price of  '+ data['rates'].newspaper_name +' ,'+ data['rates'].city_name +'</strong></div>                         <div class="form-group">	<label for="pack-price" class="col-md-6 control-label" onclick="date_pic_byid('+data['rates'].newspaper_id+');">Publish Dates of '+ data['rates'].newspaper_name +' ,'+ data['rates'].city_name +'</label> <div class="col-md-6">	<div class="box"  id="from--input">	<input class="form-control" name="p_date'+ data['rates'].newspaper_id +'" type="text" id="from-input'+ data['rates'].newspaper_id +'" data-id="'+ data['value']+'" data-toggle="modal" data-target="#dopModal" onclick="showModal('+data['rates'].newspaper_id+')" onblur="get_base_dop_price(base_id);" placeholder="mm/dd/yyyy, ..." >	</div>	<div class="code-box">	</div>	</div>	</div>                                                                       <div class="form-group"><label for="pack-price" class="col-md-3 control-label">Rate</label><div class="col-md-3"><input type="text" placeholder="" onchange="amount_calculate();" class="form-control" name="rate'+ data['rates'].newspaper_id +'" id="rate'+ data['rates'].newspaper_id +'" value="'+ data['rates'].ad_price +'"></div><label for="pack-price" class="col-md-3 control-label">B Rate</label><div class="col-md-3"><input type="text" placeholder="" onchange="amount_calculate();" class="form-control" name="brate'+ data['rates'].newspaper_id +'" id="brate'+ data['rates'].newspaper_id +'" value="0"></div></div> ';
					var p_u= document.getElementById('add_base_dates');
					 datarate=data;
					p_u.innerHTML=html;
					//p_u.insertAdjacentHTML('beforeend', html);
 ...
 function get_agencies()
            ...
            $.ajax({                
                ...
                url: "<?php echo base_url(); ?>" + "admin/booking_cd_ro/get_agency",
