	...
	<div class="row">
		<div class="col-lg-12">
			...
			<?php 
					...
					echo form_open_multipart('admin/fm_ro/edit', $attributes); ?>
					<?php
					echo "<div class='text-danger'>";
					echo validation_errors();
					echo "</div>";
					?>		
					<input type="hidden" id="ro_no" name="ro_no" value="<?php echo $fm->ro_no;?>" >
					<input type="hidden" id="id" name="id" value="<?php echo $fm->id;?>" >
					<div class="form-group" style="background-color: lightblue;">
                        ...
                        <div class="col-md-5">
							<select id="fm_group1" name="fm_group1" class="form-control" onchange="get_fmc();" required>
								<option value="">Choose FM Group</option>
								<?php foreach($fm_groups as $fm_group){  ?>
                                              	<option value="<?php echo $fm->g_id; ?>" <?php echo($fm->g_id == $fm_group->ng_id)? 'selected="selected"':''; ?> ><?php echo $fm_group->ng_name; ?> </option>
								<?php  } ?>
                            </select>
                        </div>
...
function save_data()
{
 ...
 $.ajax({ 
        	url: "<?php echo base_url(); ?>" + "admin/fm_ro/edit()",
			type: "POST",				
			async: true ,    
			data :{'td':JSON.stringify(TableData),'FMG':FM_G,'fmc':fmc,'city':city,'client':client,'ro_date':ro_date,'heading':heading,'remarks':remarks,'comm_a':comm_a,'tax_a':tax_a,'t_amount':t_amount,'p_amount':p_amount},
           dataType: 'json',
...
<script type="text/javascript">
    ...
    $("tr").click(function () {
       // var d= $(this).children('td').find('input[id="date_f_1"]').val();
      //  var d= $(this).children('td').text();
    ...
    alert(rowItems);
    $("#date_f").val(rowItems[0]);
    $("#date_t").val(rowItems[1]);
    $("#day").val(rowItems[2]);
    $("#sd").val(rowItems[3]);
    $("#day_times").val(rowItems[4]);
    $("#ts").val(rowItems[5]);
    $("#rp10").val(rowItems[6]);
    $("#slot_rate").val(rowItems[7]);
    $("#matter").val(rowItems[8]);
    $("#total").val(rowItems[9]);
    $("#comm1").val(rowItems[10]);
    $("#comm2").val(rowItems[11]);
    $("#comm_a").val(rowItems[12]);
    $("#tax").val(rowItems[13]);
   // $("#tax option[value='rowItems[13]'").attr('selected',true);
   
    $("#tax_a").val(rowItems[14]);
    $("#net_a").val(rowItems[15]);
