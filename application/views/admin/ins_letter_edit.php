			...
			<div class="block"  style="background-color:#dbe1e8;">
				...
				<!-- General Data Content -->					
					<div class="form-group">
						...
						<div class="col-md-3">
						    <input type="hidden" name="letter_id" id="letter_id" value="<?php echo $letters->id;?>">
						     <input type="hidden" name="letter_no" id="letter_no" value="<?php echo $letters->letter_no;?>">
							<input type="text" placeholder="" class="form-control" name="ro_no" id="ro_no" value="<?php echo $letters->ro_no;?>" readonly>
							<input type="hidden" name="ro_id" id="ro_id" value="<?php echo $letters->ro_id;?>">
						</div>
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							<input type="text" placeholder="" class="form-control" name="cat" id="cat" value="<?php echo $letters->heading;?>" readonly>
						</div>
					</div>
					<div class="form-group">
					    <>
						...
						<div class="col-md-9">
						    <?php
						    $scheme=explode('+',$book_ad->scheme_name);
						  //  echo var_dump($scheme);?>
							<input type="text" placeholder="" class="form-control" name="Paid" id="Paid" value="<?php echo $scheme[0];?>" readonly>
						</div>
						<label for="pack-price" class="col-md-3 control-label">Free</label>
						<div class="col-md-9">
							<input type="text" placeholder="" class="form-control" name="Free" id="Free" value="<?php echo $scheme[1];?>" readonly>
						</div>
					</div>
					<input type="hidden" name="client" id="client" value="<?php echo $letters->client_id;?>">
					<input type="hidden" name="inse" id="inse" value="<?php echo $book_ad->insertion;?>">
					<input type="hidden" name="cat1" id="cat1" value="<?php echo $book_ad->cat_id;?>">
					<input type="hidden" name="type_id" id="type_id" value="<?php echo $book_ad->type_id;?>">
					<input type="hidden" name="scheme" id="scheme" value="<?php echo $book_ad->scheme;?>">
						<input type="hidden" name="work_year" id="work_year" value="<?php echo $book_ad->work_year;?>">
			...
			success: function(data)
				{
					...
					document.getElementById("type_id").value = data.book_ad.type_id;
						scheme=data.book_ad.scheme_name;  
...
function select_ro1(id)
{

					var letters_details = $.parseJSON('<?php echo json_encode($letters_details)?>');
					var dops1 = $.parseJSON('<?php echo json_encode($dops1)?>');
					console.log(letters_details);
					console.log(dops1);
					...
					$.each(letters_details, function(i, d) 
					{	
					    var s_days="";   
					    var c="";
					    var count=0;
					...
					//	var html='<div class="alert alert-info" onclick="date_pic_byid('+d.paper_id+');" style="cursor:crosshair"><strong>  '+ d.newspaper_name +' ,'+ d.city_name +'</strong></div>  <div class="form-group">	<label for="pack-price" class="col-md-6 control-label" >Add New Publish Date</label> <div class="col-md-6">	<div class="box"  id="from--input">	<input class="form-control" name="from-input'+ d.paper_id +'[]" type="text" id="from-input'+ d.paper_id +'" placeholder="mm/dd/yyyy, ..." >	</div>	<div class="code-box">	</div>	</div>	</div>';
						var html='<div class="alert alert-info" onclick="date_pic_byid('+d.paper_id+');" style="cursor:crosshair"><strong>  '+ d.newspaper_name +' ,'+ d.city_name +'</strong></div>                         <div class="form-group">	<label for="pack-price" class="col-md-4 control-label" >Last Publish Date</label> <div class="col-md-3">	<div class="box"  id="from--input"><input class="form-control" name="ro_id'+ d.ro_id +'[]" type="text" id="ro_id'+ d.ro_id +'" value="'+d.ro_id+'" readonly>	<input class="form-control" name="last_dop'+ d.paper_id +'[]" type="text" id="last_dop'+ d.paper_id +'" value="'+d.last_dop+'" readonly>	</div>	<div class="code-box">	</div>	</div>	<label for="pack-price" class="col-md-3 control-label" >No of Already Insertion</label> <div class="col-md-2">	<div class="box"  id="frput">	<input class="form-control" name="last_ins'+ d.paper_id +'[]" type="text" id="last_ins'+ d.paper_id +'" value="'+ d.last_inse +'" readonly>	</div>	<div class="code-box"><label class="col-md-3 control-label"> Dop_Amount</label><input type="text" name="dop_amount'+d.paper_id+'[]" value="'+dop_amount+'">	</div>	</div></div>                      		    <div class="form-group">	<label for="pack-price" class="col-md-6 control-label" >Add New Publish Date</label> <div class="col-md-6">	<div class="box"  id="from--input">	<input class="form-control" name="from-input'+ d.paper_id +'[]" type="text" id="from-input'+ d.paper_id +'" onclick="date_pic_byid('+d.paper_id+')" placeholder="mm/dd/yyyy, ..." >	</div>	<div class="code-box">	</div>	</div>	</div>'
						p_u= document.getElementById('ins_dops');
					
						p_u.insertAdjacentHTML('beforeend', html);
						 var dops=d.dops.split(", ");
						 dops_count=dops.length;
						// alert("dop"+dops_count);
...
var Pleft=parseInt($("#Paid").val())-parseInt(last_ins);
//alert("Pleft"+Pleft);
var count=1;
...
// 	//var  box_c= $("#box_c").val();
 	...
 	var form_data= {'ro_no':ro_no,'ro_id':ro_id,'ro_date':ro_date,'client':client,'work_year':work_year,'sheading':sheading,'inse':inse,'scheme':scheme,'material':material,'remark':remark,'p_date':p_date,'Paid':Paid,'Free':Free,'dops_count':dops_count,'letter_no':letter_no};
	console.log(form_data);
	$.ajax({                
			url: "<?php echo base_url(); ?>" + "admin/ins_letter/edit/"+"<?php echo $letters->id; ?>",
			type: "POST",				
			async: true ,               
			data: form_data,
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
