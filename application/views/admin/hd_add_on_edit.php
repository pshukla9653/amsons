		...
		<div class="col-lg-12">
			...
			<div class="block">
				...
				<?php 
					...
					echo form_open_multipart('admin/hd_add_on/edit/'.$add_on->id, $attributes); ?>
					<?php
                        ...
                        <div class="col-md-9">
							<select id="ad_cat" name="ad_cat" class="select-chosen" data-placeholder="Choose Heading" style="width: 250px;" required>
								<?php foreach($cats as $cat){ ?>
                                        <option value="<?php echo $cat->id; ?>" <?php if($add_on->ad_cat_id==$cat->id)echo "selected";?>><?php echo $cat->position; ?></option>
								<?php }?>
                            </select>
                        </div>
                    </div>
...
function get_m_newspaper()
{
	...
	$.ajax({
			url: "<?php echo base_url(); ?>" + "admin/hd_add_on/get_newspaper",
			type: "POST",				
			async: true ,               
			data: {'news_g':news_g},
...
function get_a_newspaper()
{
	...
	$.ajax({
			url: "<?php echo base_url(); ?>" + "admin/hd_add_on/get_add_newspaper",
			type: "POST",
			async: true ,
			data: {'news_g':news_g},
			beforeSend: function(){ document.getElementById("loader").style.display = "block";},
