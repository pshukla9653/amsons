		...
		<div class="col-lg-12">
			...
			<div class="block">
				...
				<?php 
					...
					echo form_open_multipart('admin/add_on/add', $attributes); ?>
					<?php
                        ...
                        <div class="col-md-9">
							<select id="newspaper_group" name="newspaper_group" onchange="get_m_newspaper();" class="form-control" data-placeholder="Choose Newspaper Group" required>
								...
								<?php foreach($news_groups as $news_group){ ?>
                                        <option value="<?php echo $news_group->ng_id; ?>" <?php echo set_select('newspaper_group', $news_group->ng_id); ?>><?php echo $news_group->ng_name; ?></option>
								<?php }?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-md-9">
							<select id="ad_cat" name="ad_cat" class="select-chosen" data-placeholder="Choose Heading" style="width: 250px;" required>
								<?php foreach($cats as $cat){ ?>
                                        <option value="<?php echo $cat->id; ?>" <?php echo set_select('ad_cat', $cat->id); ?> ><?php echo $cat->name; ?></option>
								<?php }?>
                            </select>
                        </div>
                    </div>
				...
				<div class="col-lg-6">
					<div class="form-group">
						...
						<div class="col-md-9">
							<input type="number"  onkeyup="inse_to();"  onchange="inse_to();" placeholder=""  min="1" class="form-control" name="ins_from" id="ins_from" value="<?php echo set_value('ins_from'); ?>" required>
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
                        ...
                        <div class="col-md-9">
							<select id="mul" name="mul" class="form-control" data-placeholder="Choose Ad Page" required>
								<option value="A" <?php echo set_select('mul', 'A'); ?>>ADD</option>
								<option value="M" <?php echo set_select('mul', 'M'); ?>>Multiple</option>
                            </select>
                        </div>
                    </div>
					<div class="form-group">
                        ...
                        <div class="col-md-9">
							<select id="unit" name="unit" class="form-control" data-placeholder="" required>
								<option value="">Choose one</option>
								<option value="W" <?php echo set_select('unit', 'W'); ?>>Word</option>
								<option value="L" <?php echo set_select('unit', 'L'); ?>>Line</option>
                            </select>
                        </div>
                    </div>
                    	<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-md-3">
							<input type="number" placeholder="" onchange="get_unit(0)"; class="form-control" name="f_unit" id="f_unit" min="1" value="<?php echo set_value('f_unit'); ?>" required>
						</div>
						<label for="price" class="col-md-3 control-label">To Unit</label>
						<div class="col-md-3">
							<input type="number" placeholder="" class="form-control" name="t_unit" id="t_unit" value="<?php echo set_value('t_unit'); ?>" required>
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						<div class="col-md-9 col-md-offset-3">
							...
							<a href="<?php echo base_url()?>admin/add_on"><div class="btn btn-sm btn-primary">
...
function save()
{
	...
	var form_data= {'newspaper_group':newspaper_group,'m_newspaper':m_newspaper,'a_newspaper':a_newspaper,'ad_cat':ad_cat,'ins_from':ins_from,'ins_to':ins_to,'unit':unit,'date_f':date_from,'date_t':date_to,'f_unit':f_unit,'t_unit':t_unit,'price':price,'eprice':eprice,'mul':mul};
	
	$.ajax({
			url: "<?php echo base_url(); ?>" + "admin/add_on/save",
			type: "POST",				
			async: true ,               
			data: form_data,
