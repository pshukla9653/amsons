...
class Booking_text_ro extends CI_Controller
{ 
...
INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE (tbl_book_ads.pending='C' AND tbl_book_ads.type_id='1') AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR c.name LIKE '%".$name."%' OR u.mobile LIKE '%".$name."%' OR u.email LIKE '%".$name."%') ORDER BY id DESC");
			
			$config = array();
			$config["base_url"] = base_url() . "admin/booking_text_ro/index";
...
INNER JOIN tbl_news_type t ON t.id=tbl_book_ads.type_id
INNER JOIN tbl_categories c ON c.id=tbl_book_ads.cat_id
INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE tbl_book_ads.pending='C' AND tbl_book_ads.type_id='1' order by tbl_book_ads.id desc" );
			$config = array();
			$config["base_url"] = base_url(). "admin/booking_text_ro/index";
			$config["total_rows"] = $book_ads->num_rows();
			$config["per_page"] = 20;
...
INNER JOIN tbl_news_type t ON t.id=tbl_book_ads.type_id
INNER JOIN tbl_categories c ON c.id=tbl_book_ads.cat_id
INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE tbl_book_ads.pending='C' AND tbl_book_ads.type_id='1' order by tbl_book_ads.id desc limit {$config['per_page']} offset {$page}");
		...
		if (!empty($this->input->post())) 
		{
           ...
           $data['sgst']= $sgst->get()->row();
			...
			if ($this->form_validation->run() == FALSE) 
			{
				...
				$this->load->view('admin/header');
				$this->load->view('admin/b_text_ro_add',$data);
				$this->load->view('admin/footer');
			}
			else
			{								
				...
				if($c<$values['insertion'])
				{
					...
					redirect('admin/booking_text_ro/add');
				}								
...
INNER JOIN tbl_cities c ON c.id=tbl_paper_city.city_id ");
            ...
            $result = $this->db->get('tbl_book_ads_temp')->row();  
            $data['bid']= $result->book_id;
  ...
  public function get_price()
    {
        if (!empty($this->input->post())) 
        {
            ...
            else
            {
                ...
                $query = $this->db->query("SELECT * FROM `tbl_ad_price` WHERE `newspaper_id` = '".$values['newspaper_id']."' AND city='".$values['city']."' AND `ad_type` = '1' AND `ad_cat_id` = '".$values['cat_id']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."' AND `date_to` =''");	
...
//die;
					...
					WHERE tbl_ad_price.newspaper_id= '".$values['newspaper_id']."' AND tbl_ad_price.city='".$values['city']."' AND tbl_ad_price.ad_type = '". $values['type_id'] ."' AND tbl_ad_price.ad_cat_id = '".$values['cat_id']."' AND tbl_ad_price.ins_from <= '".$values['insertion']."' AND tbl_ad_price.ins_to >= '".$values['insertion']."' AND tbl_ad_price.date_to =''");
//echo $this->db->last_query();
//die;
	...
	public function commission($id)
	{
		if (!empty($this->input->post())) 
		{
			...
			redirect('admin/booking_text_ro');
	...
	{		
		if (!empty($this->input->post())) 
		{
			...
			if ($this->form_validation->run() == FALSE)
			{
				redirect('admin/booking_text_ro/edit/'.$id);
...
INNER JOIN tbl_cities c ON c.id=tbl_paper_city.city_id ");
	...
	public function del($id)
	{
		...
		$ro= $query->row();
		$old_amount =$ro->ad_cost;
		$this->db->delete("tbl_ro-dop","id=".$id);
	...
	{
		$book_ads = $this->db->query("SELECT tbl_book_ads.*, n.name as newspaper_name, t.name as type_name, c.name as cat_name, e.e_name, u.email, u.mobile, u.client_name FROM tbl_book_ads
INNER JOIN tbl_newspapers n ON n.id=tbl_book_ads.newspaper_id
INNER JOIN tbl_news_type t ON t.id=tbl_book_ads.type_id
INNER JOIN tbl_categories c ON c.id=tbl_book_ads.cat_id
