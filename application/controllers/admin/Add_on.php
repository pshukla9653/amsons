...
class Add_on extends CI_Controller
{
	...
	public function index()
	{
		...
		if(!empty($this->input->post('name')))
		{
			...
			$query = $this->db->query("SELECT tbl_add_on.*,c.name as cat_name,ng.ng_name,n.name,an.name as add_name FROM tbl_add_on
			INNER JOIN tbl_news_group ng ON ng.ng_id=tbl_add_on.g_id
			INNER JOIN tbl_paper_city pc ON pc.id=tbl_add_on.m_paper_id
			INNER JOIN tbl_categories c ON c.id=tbl_add_on.ad_cat_id
			INNER JOIN tbl_paper_city cp ON cp.id=tbl_add_on.a_paper_id
			INNER JOIN tbl_newspapers an ON an.id=cp.paper_id
			INNER JOIN tbl_newspapers n ON n.id=pc.paper_id WHERE tbl_add_on.ad_type='1' AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR c.name LIKE '%".$name."%')");
			
			$config["base_url"] = base_url() . "admin/add_on/index";
		...
		else
		{
			...
			INNER JOIN tbl_paper_city cp ON cp.id=tbl_add_on.a_paper_id
			INNER JOIN tbl_newspapers an ON an.id=cp.paper_id
			INNER JOIN tbl_newspapers n ON n.id=pc.paper_id WHERE tbl_add_on.ad_type='1'");
			...
			INNER JOIN tbl_paper_city cp ON cp.id=tbl_add_on.a_paper_id
			INNER JOIN tbl_newspapers an ON an.id=cp.paper_id
			INNER JOIN tbl_newspapers n ON n.id=pc.paper_id WHERE tbl_add_on.ad_type='1' order by tbl_add_on.id desc limit {$config['per_page']} offset {$page}");
			$data['adds']= $query->result();
		}		
		...
		$this->load->view('admin/add_on_list', $data);
		$this->load->view('admin/footer');
		
		
		/*
		$adds = $this->db->query("SELECT tbl_add_on.*,c.name as cat_name,ng.ng_name,n.name FROM tbl_add_on
		INNER JOIN tbl_news_group ng ON ng.ng_id=tbl_add_on.g_id
		INNER JOIN tbl_paper_city pc ON pc.id=tbl_add_on.newspaper_id
		INNER JOIN tbl_categories c ON c.id=tbl_add_on.ad_cat_id
		INNER JOIN tbl_newspapers n ON n.id=pc.paper_id");

		$data['adds']= $adds->result();
		$data["total_rows"] = $adds->num_rows();
	...
	public function add()
	{
		if (!empty($this->input->post())) 
		{
			...
			else
			{
				...
				{
					$values['newspaper_id']=$newspaper;
					
					$add = $this->db->query("SELECT * FROM tbl_add_on WHERE g_id='".$values['g_id']."' AND newspaper_id='".$values['newspaper_id']."' AND ad_cat_id='".$values['ad_cat_id']."'");
					...
					$adds=$add->result();
						...
						{							
							...
							$this->session->set_flashdata('msg', 'Add on add successfully');
						}
						else
						{
							$add_id=0;
							...
							foreach($adds as $add)
							{
								if(($add->ins_to < $values['ins_from'] OR $add->ins_from > $values['ins_to']))
								{
									continue;
								}
								else
		...
		else
		{
		   ...
		   // $date_t=date_create($this->input->post('date_t'));
			$values = array(
				...
				'ins_to'=>$this->input->post('ins_to'),
				'unit'=>$this->input->post('unit'),
				'date_from'=>date("Y-m-d",strtotime($date_f)),
			...
			foreach($newspapers as $newspaper)
			{
				...
				$adds=$add->result();
					...
					else
					{
						...
						{
							$m='Add on already Added with some Newspapers';
						}							
					}
	...
	public function get_newspaper()
	{
		...
		INNER JOIN tbl_cities c ON c.id=tbl_paper_city.city_id WHERE n.g_id='".$g_id."' ");//AND n.media_type='1'
	...
	public function edit($id)
	{
		...
		{			
			...
			if($this->form_validation->run() == FALSE) 
			{
				...
				$this->load->view('admin/header');
				$this->load->view('admin/add_on_edit',$data);
				$this->load->view('admin/footer');
			}
			else
			{	
				...
				else
				{
					...
					foreach($add_ons as $add)
					{
						...
						{
							//if(($add_on->ins_to < $values['ins_from'] OR $add_on->ins_from > $values['ins_to']))
							if(($add->ins_to < $values['ins_from'] OR $add->ins_from > $values['ins_to']) OR ($add->t_unit < $values['f_unit'] OR $add->f_unit > $values['t_unit']))
							{
								continue;
							}
