...
class Hd_r_rate extends CI_Controller
{
...
INNER JOIN tbl_position p ON p.id=tbl_ad_price.ad_cat_id WHERE (tbl_ad_price.ad_type='3') AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR p.position LIKE '%".$name."%') order by tbl_ad_price.id desc");

			$config["base_url"] = base_url() . "admin/hd_r_rate/index";
...
INNER JOIN tbl_position p ON p.id=tbl_ad_price.ad_cat_id WHERE tbl_ad_price.ad_type='3' order by tbl_ad_price.id desc limit {$config['per_page']} offset {$page}");
		...
		$this->load->view('admin/hd_r_rate_list', $data);
...
INNER JOIN tbl_cities c ON c.id=tbl_paper_city.city_id");
				...
				$this->load->view('admin/header');
				$this->load->view('admin/hd_r_rate_set',$data);
				$this->load->view('admin/footer');
			}
			else			
			{
