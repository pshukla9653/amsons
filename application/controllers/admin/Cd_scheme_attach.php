...
class Cd_scheme_attach extends CI_Controller
{
	...
	public function index()
	{
		...
		if(!empty($this->input->post('name')))
		{
			...
			$query = $this->db->query("SELECT tbl_paper_scheme.*, n.name as newspaper_name, c.name as cat_name ,city.name as city_name, d.day as day_name FROM tbl_paper_scheme INNER JOIN tbl_paper_city pc ON pc.id=tbl_paper_scheme.np_id INNER JOIN tbl_cities city ON city.id=pc.city_id INNER JOIN tbl_newspapers n ON n.id=pc.paper_id 
			INNER JOIN tbl_days d ON d.id=tbl_paper_scheme.days
			INNER JOIN tbl_categories c ON c.id=tbl_paper_scheme.cat_id WHERE tbl_paper_scheme.type_id='2' AND (n.name LIKE '%".$name."%' OR c.name LIKE '%".$name."%' OR d.day LIKE '%".$name."%' OR city.name LIKE '%".$name."%')");
			
			$config["base_url"] = base_url() . "admin/cd_scheme_attach/index";
		...
		{
			$query = $this->db->get('tbl_paper_scheme');
			$data['schemes']= $query->result();
			...
			$query = $this->db->query("SELECT tbl_paper_scheme.*, n.name as newspaper_name, c.name as cat_name ,city.name as city_name, d.day as day_name FROM tbl_paper_scheme INNER JOIN tbl_paper_city pc ON pc.id=tbl_paper_scheme.np_id INNER JOIN tbl_cities city ON city.id=pc.city_id INNER JOIN tbl_newspapers n ON n.id=pc.paper_id 
			INNER JOIN tbl_days d ON d.id=tbl_paper_scheme.days
			INNER JOIN tbl_categories c ON c.id=tbl_paper_scheme.cat_id WHERE tbl_paper_scheme.type_id='2' order by tbl_paper_scheme.id desc limit {$config['per_page']} offset {$page}");
			$data['schemes']= $query->result();
		}		
		...
		$this->load->view('admin/cd_scheme_attach_list', $data);
...
INNER JOIN tbl_cities c ON c.id=tbl_paper_city.city_id ");
				...
				$this->load->view('admin/header');
				$this->load->view('admin/cd_scheme_attach', $data);
				$this->load->view('admin/footer');
			}
			else
			{
				...
				$add_id='';
					...
					foreach($schemes as $scheme)
					{
						...
						foreach($cats as $cat)
						{
							foreach($days as $day)
							{
								$query = $this->db->query("SELECT * FROM `tbl_paper_scheme` WHERE `scheme_id`='".$scheme."' AND `np_id`='".$values['np_id']."' AND `cat_id`='".$cat."' AND  `type_id`='2' AND `days`='".$day."'");
								$scheme=$query->row();
								if($scheme==NULL)
								{
									...
									$values['days']=$day;
									$this->db->insert('tbl_paper_scheme', $values);
								}
								else
								{
				...
				if($f==0)
				{
					...
					redirect('admin/cd_scheme_attach');
				}
				else
				{
...
INNER JOIN tbl_cities c ON c.id=tbl_paper_city.city_id ");
	...
	{
		if($this->db->delete("tbl_paper_scheme",array('id' => $id)))
		{
