...
class R_rate extends CI_Controller
{
...
INNER JOIN tbl_categories c ON c.id=tbl_ad_price.ad_cat_id WHERE (tbl_ad_price.ad_type='1') AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR c.name LIKE '%".$name."%') order by tbl_ad_price.id desc");
			...
			$config["base_url"] = base_url() . "admin/r_rate/index";
...
INNER JOIN tbl_categories c ON c.id=tbl_ad_price.ad_cat_id WHERE tbl_ad_price.ad_type='1' order by tbl_ad_price.id desc limit {$config['per_page']} offset {$page}");
		...
		$this->load->view('admin/r_rate_list', $data);
...
INNER JOIN tbl_categories c ON c.id=tbl_cat_with_paper.cat_id WHERE tbl_cat_with_paper.newspaper_id ='".$ap[0]->newspaper_id."'");
			...
			//$data['cats']= $query->result();
				...
				$this->load->view('admin/header');
				$this->load->view('admin/r_rate_set',$data);
				$this->load->view('admin/footer');
			}
			else			
			{
