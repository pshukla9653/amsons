...
class Booking_cd_ro extends CI_Controller
{ 
...
INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE (tbl_book_ads.pending='C' AND tbl_book_ads.type_id='2') AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR c.name LIKE '%".$name."%' OR u.mobile LIKE '%".$name."%' OR u.email LIKE '%".$name."%') ORDER BY id DESC");
			
			$config = array();
			$config["base_url"] = base_url() . "admin/booking_cd_ro/index";
...
INNER JOIN tbl_news_type t ON t.id=tbl_book_ads.type_id
INNER JOIN tbl_categories c ON c.id=tbl_book_ads.cat_id
INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE tbl_book_ads.pending='C' AND tbl_book_ads.type_id='2' order by tbl_book_ads.id desc" );
			$config = array();
			$config["base_url"] = base_url(). "admin/booking_cd_ro/index";
			$config["total_rows"] = $book_ads->num_rows();
			$config["per_page"] = 20;
...
INNER JOIN tbl_news_type t ON t.id=tbl_book_ads.type_id
INNER JOIN tbl_categories c ON c.id=tbl_book_ads.cat_id
INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE tbl_book_ads.pending='C' AND tbl_book_ads.type_id='2' order by tbl_book_ads.id desc limit {$config['per_page']} offset {$page}");
	...
	public function add()
	{
		...
		if (!empty($this->input->post())) 
		{
			...
			if ($this->form_validation->run() == FALSE) 
			{
				...
				$this->load->view('admin/header');
				$this->load->view('admin/b_cd_ro_add',$data);
				$this->load->view('admin/footer');
			}
			else
			{								
...
public function get_price()
 ...
 //$paper=$p_id;
    ...
    public function get_dop_price()
    {
        ...
        $query = $this->db->query("SELECT * FROM `tbl_ad_price` WHERE `newspaper_id` = '".$values['newspaper_id']."' AND city='".$values['city']."' AND `ad_type` = ' 2 ' AND `ad_cat_id` = '".$values['cat_id']."'  AND `color_type` = '".$values['color']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."' AND `date_to` =''");
 ...
 public function get_ad_on_price()
	...
	public function edit($id,$go="")
	{
		if (!empty($this->input->post())) 
		{
			...
			if ($this->form_validation->run() == FALSE)
			{
				redirect('admin/booking_cd_ro/edit/'.$id);
				/*
