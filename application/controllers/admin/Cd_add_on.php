...
class Cd_add_on extends CI_Controller
{
...
INNER JOIN tbl_categories c ON c.id=tbl_ad_price.ad_cat_id WHERE (tbl_ad_price.ad_type='1') AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR c.name LIKE '%".$name."%') order by tbl_ad_price.id desc");
...
INNER JOIN tbl_newspapers n ON n.id=pc.paper_id WHERE tbl_add_on.ad_type='2'");

			$config["base_url"] = base_url(). "admin/cd_add_on/index";
			$config["total_rows"] = $query->num_rows();
			$config["per_page"] = 20;
			$config["uri_segment"] = 4;
...
INNER JOIN tbl_paper_city cp ON cp.id=tbl_add_on.a_paper_id
INNER JOIN tbl_newspapers an ON an.id=cp.paper_id
INNER JOIN tbl_newspapers n ON n.id=pc.paper_id WHERE tbl_add_on.ad_type='2' order by tbl_add_on.id desc limit {$config['per_page']} offset {$page}");
			$data['adds']= $query->result();
		}		
		...
		$this->load->view('admin/cd_add_on_list', $data);
	...
	public function add()
	{
		...
		$this->load->view('admin/header');
		$this->load->view('admin/cd_add_on_add',$data);
		$this->load->view('admin/footer');
	...
	public function edit($id)
	{
		...
		{			
			...
			if($this->form_validation->run() == FALSE) 
			{
				...
				$this->load->view('admin/header');
				$this->load->view('admin/cd_add_on_edit',$data);
				$this->load->view('admin/footer');
			}
			else
			{			
