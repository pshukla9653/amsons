...
class Newspaper extends CI_Controller
{
    ...
    public function index()
    {
       	...
       	if(!empty($this->input->post('name')))
		{
			...
			$news_address=$this->db->query("SELECT * from tbl_news_group as Ng  LEFT OUTER JOIN tbl_news_group_details as ngd ON Ng.ng_id=ngd.newsgroup_id LEFT JOIN tbl_cities as tc ON tc.id=ngd.state order by Ng.ng_id asc");
    ...
    public function show()
    {
        if(!empty($this->input->post()))
        {
            ...
            if(!empty($this->input->post('news_group')))
                {
                    $gid=$this->input->post('news_group');
                            if(!empty($this->input->post('name')))
                        {
                            $name=$this->input->post('name');
                            $newspaper = $this->db->query("SELECT * from tbl_newspapers WHERE name LIKE '%".$name."%' AND g_id='".$gid."'");
                        }
                        else
                        {
                            $gid=$this->input->post('news_group');
          
                            $newspaper = $this->db->query("SELECT * from tbl_newspapers WHERE  g_id='".$gid."'");
                        }
                }
                ...
                $newspaper = $this->db->query("SELECT * from tbl_newspapers WHERE name LIKE '%".$name."%'");
                }
                 $data['newspapers']=array();
                $c=0;
                $data['result']=$newspaper->result();
                ...
                foreach($data['result'] as $row){
                 ...
                 $newspaper_groups = $this->db->get('tbl_news_group')->result();
        $data['newspaper_group'] = $newspaper_groups;
        }
        else
        {
            $newspaper_groups = $this->db->get('tbl_news_group')->result();
...
//die;
    ...
    public function edit_newspaper($id=null)
    {
        if (!empty($this->input->post())) 
        {		
            //echo "<pre>"; var_dump($_POST); die;
            ...
            else
            {
                ...
                $query=$this->db->query("select city_id from tbl_paper_city where paper_id='$p_id'");
                $res=$query->result();
                $cityarray=array();
                
               foreach($res as $re)
               {
                  array_push($cityarray,$re->city_id);
               }
             //  echo var_dump($cityarray);
                     ...
                     foreach($this->input->post('cities[]') as $city)
                        { 
                            ...
                            else
                            {
                         	...
                         	$this->db->insert('tbl_paper_city', $values1,"paper_id =".$id);
                            }
                         }
                       //   die;
                // }
                $this->db->update('tbl_newspapers',$values, "id =".$id);
                // $this->db->update('tbl_paper_city',$values, "id =".$id);
                //echo $this->db->last_query(); die;
