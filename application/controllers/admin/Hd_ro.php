...
class Hd_ro extends CI_Controller
{ 
    ...
    public function index()
    {
        if(!empty($this->input->post('name')))
        {
			...
			INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE (tbl_book_ads.pending='C' AND tbl_book_ads.type_id='3' AND tbl_book_ads.work_year='".$_SESSION['admin']['work_year']."') AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR p.position LIKE '%".$name."%' OR u.mobile LIKE '%".$name."%' OR u.email LIKE '%".$name."%') ORDER BY id DESC");

            $config = array();
            $config["base_url"] = base_url() . "admin/hd_ro/index";
        ...
        {
            $query="SELECT tbl_book_ads.*, n.name as newspaper_name, t.name as type_name, p.position as cat_name, u.email, u.mobile FROM tbl_book_ads
			INNER JOIN tbl_newspapers n ON n.id=tbl_book_ads.newspaper_id
			INNER JOIN tbl_news_type t ON t.id=tbl_book_ads.type_id
			INNER JOIN tbl_position p ON p.id=tbl_book_ads.cat_id
			INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE tbl_book_ads.type_id='3' AND tbl_book_ads.work_year='".$_SESSION['admin']['work_year']."' order by tbl_book_ads.id desc";
            //INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE tbl_book_ads.pending='C' AND tbl_book_ads.type_id='3' AND tbl_book_ads.work_year='".$_SESSION['admin']['work_year']."' order by tbl_book_ads.id desc"; 
            $book_ads = $this->db->query($query);
            $config = array();
            $config["base_url"] = base_url(). "admin/hd_ro/index";
            $config["total_rows"] = $book_ads->num_rows();
            $config["per_page"] = 20;
...
$query=$this->db->where(["tax_type"=>"newspaper"])->get("tbl_tax")->row();
        ...
        $this->load->view('admin/header');
        $this->load->view('admin/hd_ro_add',$data);
        $this->load->view('admin/footer');
        ...
        public function get_size()
    {
        if (!empty($this->input->post())) 
        {
                ...
                $query = $this->db->query("SELECT * FROM `tbl_ad_price` WHERE `newspaper_id` = '".$newspaper."' AND color_type='".$color."' and `ad_cat_id` = '".$cat_id."' and size_type='F' ");
    ...
    public function get_price()
    {
        if (!empty($this->input->post())) 
        {
            ...
            else
            {
                ...
                $query = $this->db->query("SELECT * FROM `tbl_ad_price` WHERE `newspaper_id` = '".$values['newspaper_id']."' AND city='".$values['city']."' AND `ad_type` = '3' AND `ad_cat_id` = '".$values['cat_id']."' AND `color_type` = '".$values['color']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."'");
    ...
    public function get_fdays()
    {
        if (!empty($this->input->post())) 
        {
            ...
            'color'=> $this->input->post('color'),
                ...
                $query = $this->db->query("SELECT * FROM `tbl_ad_price` WHERE `newspaper_id` = '".$values['newspaper_id']."' AND city='".$values['city']."' AND `ad_type` = '". 3 ."' AND `ad_cat_id` = '".$values['cat_id']."' AND `color_type` = '".$values['color']."'");	
    ...
    public function get_dop_price()
    {
        ...
        foreach($rates as $rate)
        {
            ...
            {
                if($rate->date_from <= $values['s_date'] AND $rate->date_to ="0000-00-00" )
                {
                    $rate1=$rate;
                }
                else
...
$dop_amt_a = ($addon - (($addon * $values['dis_per'])/100))/($inse_dop);
      ...
      //  echo var_dump($publish_date);
               ...
               if($values['ro_type']=="M" && $odf==0)
                ...
                $client= $query->row();
//echo ($client->credit_bal+$values['ad_cost']);
//echo $client->credit_limit;
 ...
 if($values['pending']=='C'){  
  ...
  $msg="4";
            ...
            //    $book_ads=$this->db->query("SELECT tbl_book_ads.*,t.name as type_name, po.position as cat_name, u.email, u.mobile, u.client_name,d.dop FROM tbl_book_ads INNER JOIN tbl_ro_dop d ON d.ro_id=tbl_book_ads.id INNER JOIN tbl_news_type t ON t.id=tbl_book_ads.type_id INNER JOIN tbl_position po ON po.id=tbl_book_ads.cat_id INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE tbl_book_ads.id'".$in_id."'");
}
else
{
    //	$book_ads=$this->db->query("SELECT tbl_book_ads.*,t.name as type_name, po.position as cat_name, u.email, u.mobile, u.client_name,d.dop FROM tbl_book_ads INNER JOIN tbl_ro_dop d ON d.ro_id=tbl_book_ads.id INNER JOIN tbl_news_type t ON t.id=tbl_book_ads.type_id INNER JOIN tbl_position po ON po.id=tbl_book_ads.cat_id INNER JOIN tbl_client u ON u.id=tbl_book_ads.u_id WHERE tbl_book_ads.id'".$in_id."'");
...
INNER JOIN tbl_categories c ON c.id=tbl_cat_with_paper.cat_id WHERE tbl_cat_with_paper.newspaper_id ='".$id."'");*/
   ...
   //   echo $this->db->last_query();
    ...
    public function get_scheme_price()
    {
        ...
        $query = $this->db->get_where('tbl_hd_scheme', array('scheme_id' => $id));
        $scheme= $query->row();
...
*/
 $query = $this->db->query("SELECT * FROM `tbl_package` WHERE `g_id`='".$ng->g_id ."' AND `cat_id`='". $cat_id ."' AND type_id='3' AND `ins_from` <= '".$insertion."' AND `ins_to` >= '".$insertion."'");
        $pack= $query->result();
      //  echo "SELECT * FROM `tbl_package` WHERE `g_id`='".$ng->g_id ."' AND `cat_id`='". $cat_id ."' AND type_id='3' AND `ins_from` <= '".$insertion."' AND `ins_to` >= '".$insertion."'";
         ...
         $id=$this->input->post('m_newspaper');
               ...
               // $query = $this->db->query("SELECT * FROM tbl_add_on WHERE (`m_paper_id` = '".$values['m_newspaper_id']."' AND `a_paper_id` = '".$values['a_newspaper_id']."' AND `ad_type` = '". $values['type_id'] ."' AND `ad_cat_id` = '".$values['cat_id']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."' ) OR (`m_paper_id` = '".$values['a_newspaper_id']."' AND `a_paper_id` = '".$values['m_newspaper_id']."' AND `ad_type` = '". $values['type_id'] ."' AND `ad_cat_id` = '".$values['cat_id']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."' )");
                ...
                if(empty($rates))
                {
                    ...
                    $query = $this->db->query("SELECT id, `ad_price` as price, `extra_price` as e_price,`f_rate` FROM `tbl_ad_price` WHERE `newspaper_id` = '".$np->paper_id ."' AND city='".$np->city_id ."' AND `ad_type` = '". 3 ."' AND `ad_cat_id` = '".$values['cat_id']."'AND color_type = '".$values['color']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."'");
     ...
     public function get_ad_on_price1()
    {
        if (!empty($this->input->post())) 
        {
            ...
            else
            {
               ...
               // $query = $this->db->query("SELECT * FROM tbl_add_on WHERE (`m_paper_id` = '".$values['m_newspaper_id']."' AND `a_paper_id` = '".$values['a_newspaper_id']."' AND `ad_type` = '". $values['type_id'] ."' AND `ad_cat_id` = '".$values['cat_id']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."' ) OR (`m_paper_id` = '".$values['a_newspaper_id']."' AND `a_paper_id` = '".$values['m_newspaper_id']."' AND `ad_type` = '". $values['type_id'] ."' AND `ad_cat_id` = '".$values['cat_id']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."' )");
 $query = $this->db->query("SELECT * FROM tbl_add_on WHERE (`m_paper_id` = '".$values['m_newspaper_id']."' AND `a_paper_id` = '".$values['a_newspaper_id']."' AND `ad_type` = '". $values['type_id'] ."' AND `ad_cat_id` = '".$values['cat_id']."' AND `color_type`='".$values['color']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."' ) ");

$SS="SELECT * FROM tbl_add_on WHERE (`m_paper_id` = '".$values['m_newspaper_id']."' AND `a_paper_id` = '".$values['a_newspaper_id']."' AND `ad_type` = '". $values['type_id'] ."' AND `ad_cat_id` = '".$values['cat_id']."' AND `color_type`='".$values['color']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."' ) ";
                $rates=$query->row();
                ...
                if(empty($rates))
                {
                 ...
                 $query = $this->db->query("SELECT id, `ad_price` as price, `extra_price` as e_price FROM `tbl_ad_price` WHERE `newspaper_id` = '".$np->paper_id ."' AND city='".$np->city_id ."' AND `ad_type` = '". 3 ."' AND `ad_cat_id` = '".$values['cat_id']."' AND `color_type`='".$values['color']."' AND `ins_from` <= '".$values['insertion']."' AND `ins_to` >= '".$values['insertion']."'");
 ...
 $ro_no=$this->input->post('ro_no');
                $values = array(
                   ...
                   'city' => $ng->city_id,
                    ...
                    'box_charge'=>$this->input->post('box_c'),
                    'add_on_a'=>$this->input->post('add_a'),
                    'book_date'=>date_format(date_create($this->input->post('book_date')),"Y-m-d"),
                    'status' => ($this->input->post('status') == 'on') ? 'A' : 'A'
                    //                    'book_date'=>date('Y-m-d'),
                    //                    'status' => ($this->input->post('status') == 'on') ? 'A' : 'A',
                    //                    'c_date' => date('Y-m-d H:i:s')
...
INNER JOIN tbl_newspapers n ON n.id=pc.paper_id WHERE tbl_ro_dop.ro_id = '".$id."'" );
    ...
    public function ro($id)
    {
        ...
        $this->load->view('admin/header');
        $this->load->view('admin/hd_ro_generate',$data);
        $this->load->view('admin/footer');
    }
