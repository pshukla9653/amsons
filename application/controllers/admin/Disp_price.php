...
class Disp_price extends CI_Controller
{
...
INNER JOIN tbl_categories c ON c.id=tbl_ad_price.ad_cat_id WHERE (tbl_ad_price.ad_type='2') AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR c.name LIKE '%".$name."%')");
...
INNER JOIN tbl_categories c ON c.id=tbl_ad_price.ad_cat_id WHERE tbl_ad_price.ad_type='2' order by tbl_ad_price.id desc ");//limit {$config['per_page']} offset {$page}
		...
		$this->load->view('admin/disp_price_list', $data);
...
INNER JOIN tbl_cities c ON c.id=tbl_paper_city.city_id");
	...
	public function ad_days()
	{
	    $query= $this->db->query("select newspaper_id,city,ad_type,ad_cat_id from tbl_ad_price");
	    $value=$query->result();
