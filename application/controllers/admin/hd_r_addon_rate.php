...
class hd_r_addon_rate extends CI_Controller
{
	...
	public function index()
	{
		...
		if(!empty($this->input->post('name')))
		{
			...
			$query = $this->db->query("SELECT tbl_add_on.*, n.name as newspaper_name, m.name as newspaper,t.name as type_name, c.position as cat_name FROM tbl_add_on INNER JOIN tbl_newspapers n ON n.id=tbl_add_on.m_paper_id INNER JOIN tbl_news_type t ON t.id=tbl_add_on.ad_type INNER JOIN tbl_newspapers m ON m.id=tbl_add_on.a_paper_id INNER JOIN tbl_position c ON c.id=tbl_add_on.ad_cat_id WHERE (tbl_add_on.ad_type='3')
AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR c.position LIKE '%".$name."%') order by tbl_add_on.id desc");
			
			$config["base_url"] = base_url() . "admin/hd_r_addon_rate/index";
...
");
 ...
 echo $this->db->last_query();die();
		...
		$this->load->view('admin/hd_r_addon_rate_list', $data);
	...
	public function revise($id)
	{
		...
		{			
			...
			//$query = $this->db->query("SELECT * FROM `tbl_paper_city` WHERE `paper_id`='".$ap[0]->newspaper_id ."' AND `city_id`='".$ap[0]->city ."'");
				$query =$this->db->get_where('tbl_position', array('id' => $ap[0]->ad_cat_id));
...
INNER JOIN tbl_cities c ON c.id=tbl_paper_city.city_id");
				...
				$query = $this->db->query("SELECT tbl_cat_with_paper.*, n.name as newspaper_name, c.position as cat_name FROM tbl_cat_with_paper 
INNER JOIN tbl_newspapers n ON n.id=tbl_cat_with_paper.newspaper_id
INNER JOIN tbl_position c ON c.id=tbl_cat_with_paper.cat_id WHERE tbl_cat_with_paper.newspaper_id ='".$ap[0]->newspaper_id."'");
			...
			//$data['cats']= $query->result();
				...
				$this->load->view('admin/header');
				$this->load->view('admin/hd_r_addon_rate_set',$data);
				$this->load->view('admin/footer');
			}
			else			
			{
			...
			$values = array(
				...
				//	{
						...
						$query = $this->db->query("SELECT * FROM tbl_add_on WHERE g_id='".$values['g_id']."' AND m_paper_id='".$values['m_paper_id']."' AND a_paper_id='".$values['a_paper_id']."' AND ad_type='".$values['ad_type']."' AND ad_cat_id='".$values['ad_cat_id']."' and ins_to='".$values['ins_to']."' AND ins_from='".$values['ins_from']."'AND f_unit='".$values['f_unit']."' AND t_unit='".$values['t_unit']."'");
			...
			//	$this-> db->insert('tbl_add_on', $values);
				...
				redirect('admin/hd_r_addon_rate');				
			}
		}
		else
		{
...
INNER JOIN tbl_cities c ON c.id=tbl_paper_city.city_id");
	...
	public function get_heading11111()
	{
		...
		$query = $this->db->query("SELECT tbl_cat_with_paper.*, n.name as newspaper_name, c.position as cat_name FROM tbl_cat_with_paper 
INNER JOIN tbl_newspapers n ON n.id=tbl_cat_with_paper.newspaper_id
INNER JOIN tbl_position c ON c.id=tbl_cat_with_paper.cat_id WHERE tbl_cat_with_paper.newspaper_id ='".$id."'");
