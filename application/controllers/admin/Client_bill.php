...
// 	}
        					...
        					$rem=$query->result();
        					       ...
        					       // echo var_dump($dates);
        					        foreach($dates as $dt)
        					        {
                					          ...
                					          foreach($rem as $re)
                					          {
                					                    $this->db->query("UPDATE `tbl_ro_dop` set `bill_dop`='$dt1', bill_status='Y',bill_number='$bill_no',bill_amount='$p_amount' where  `id`=$re->id  and (dop='".$dt1."' OR dop='".$dt2."')  ");
                					      //	echo $this->db->last_query();
                					          }
		...
		$query=$this->db->query("delete from `tbl_bill_temp` where  emp_id='".$_SESSION['admin']['id']."' ");
			   ...
			   else
			   {
				...
				//	echo "result id:".$result['ledger_id'];
					...
					$_POST['amount'] = !empty($this->input->post('amount'))?$this->input->post('amount'):0;
					$_POST['discount'] = !empty($this->input->post('discount'))?$this->input->post('discount'):0;
						$_POST['igst'] = !empty($this->input->post('igst'))?$this->input->post('igst'):0;
					$_POST['sgst'] = !empty($this->input->post('sgst'))?$this->input->post('sgst'):0;
					$_POST['cgst'] = !empty($this->input->post('cgst'))?$this->input->post('cgst'):0;
						$_POST['box_c'] = !empty($this->input->post('box_c'))?$this->input->post('box_c'):0;
						$_POST['at_w'] = !empty($this->input->post('at_w'))?$this->input->post('at_w'):0;
						$_POST['other_c'] = !empty($this->input->post('other_c'))?$this->input->post('other_c'):0;
					...
					$values = array(
						...
						'voucher_date'=> $bill_date,
						'entry_type'=>'dr',
						'amount'=>!empty($this->input->post('amount'))?$this->input->post('amount'):0-!empty($this->input->post('discount'))?$this->input->post('discount'):0+$this->input->post('igst')+$this->input->post('sgst')+$this->input->post('cgst')+$this->input->post('box_c')+$this->input->post('at_w')+$this->input->post('other_c'),
					...
					$this->site_model->insert_data('tbl_vouchers', $values); 
				//	echo $this->db->last_query(); 
                    $ledger_name = !empty($ledger_name)?$ledger_name:'';
					$values = array(
						'group_id'=>9,
						'ledger_id'=>12,
						'voucher_date'=> $bill_date,
                                    		    ...
                                    		    { 
                                    		       ...
                                    		       // $values1['insertion']=1;
                                    				            ...
                                    				            if($size_type=="F")
                                    				                ...
                                    				                {
                                    				            	$word_size=explode('X',$ro->size_words);
                                    				            	$len=$word_size[0];
                                    				            	$width=$word_size[1];
                                    				            	$c_amount+=floatval($ro->price)*floatval($len)*floatval($width)*(1);
                                    				                }
            		...
            		//	echo $this->db->last_query();
            			...
            			$this->db->query("DELETE from tbl_vouchers where group_id= '15' and screen='Client Bill' and ledger_id='15' and screen_id='$in_id'");
            					...
            					$values = array(
            						'group_id'=>2,
            						'ledger_id'=> $ledger_id,
            						'voucher_date'=> date("Y-m-d H:i:s",strtotime($this->input->post('date_t'))),
            						'entry_type'=>'dr',
            						'amount'=>$amount-$this->input->post('discount')+$igst+$sgst+$cgst+$this->input->post('box_c')+$this->input->post('at_w')+$this->input->post('other_c'),
            						'narration'=>"Bill generated for ".$client_name."No: ".$bill_no,
            						'voucher_no'=>1,
 ...
 $this->db->query("DELETE from tbl_vouchers where group_id= '2' and screen='Client Bill' and screen_id='$bill_id'");
				...
				if($this->input->post('sgst')){
					...
					$result=$tbl_bill_temp->result();
								$this->db->select( 'tbl_bill_details.*, tbl_bill_details.rate as price,tbl_bill_details.e_rate as eprice, tbl_book_ads.ro_no as ro_no,tbl_bill.client_id as client_id,tbl_bill_temp.payable_amount as payable_amount,tbl_client.client_name as client_name,tbl_newspapers.name as newspaper_title,tbl_categories.name as cat_title,tbl_bill.bill_date');
			$this->db->from('tbl_bill_details');
			$this->db->join('tbl_book_ads', 'tbl_book_ads.id = tbl_bill_details.ro_id',left);
			$this->db->join('tbl_bill', 'tbl_bill.id = tbl_bill_details.bill_id',left);
		    $this->db->join('tbl_client', 'tbl_client.id = client_id');
			...
			$this->db->join('tbl_bill_temp', 'tbl_bill_temp.bill_id = tbl_bill_details.bill_id',left);
			$this->db->distinct();
			$query = $this->db->where(array("tbl_bill_details.bill_id"=>$in_id))->get();
			$bill_detail_result= $query->result();
			$p_amount=0;
                                                                
			foreach($bill_detail_result as $ros){
				$p_amount = $ros->net_amount;
			} 
					...
					foreach($bill_data as $bd){	
        					...
        					$rem=$query->result();
        					        ...
        					        foreach($dates as $dt)
        					        {
                					    ...
                					    $dd = str_replace("/","-",$dt);
                					           $dt1=date("Y-m-d",strtotime($dd));
                					          foreach($rem as $re)
                					          {
                					              $p_amount = !empty($p_amount)?$p_amount:0;
                					                    $this->db->query("UPDATE `tbl_ro_dop` set `bill_dop`='$dt1', bill_status='Y',bill_number='$bill_no',bill_amount='$p_amount' where  `id`=$re->id  and (dop='".$dt1."')  ");
                	                            // echo $this->db->last_query(); die; 	
                					          }
