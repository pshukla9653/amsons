...
class Generate_ro extends CI_Controller
{ 
	...
	public function index()
	{
		if(!empty($this->input->post('name')))
		{
			...
			$data['name']= $name;
			$book_ads=$this->db->query("SELECT tbl_book_ads_temp.*, n.name as newspaper_name, t.name as type_name, c.name as cat_name, u.email, u.mobile, u.client_name FROM tbl_book_ads_temp
...
INNER JOIN tbl_client u ON u.id=tbl_book_ads_temp.u_id WHERE (tbl_book_ads_temp.pending='C' AND tbl_book_ads_temp.type_id='1' and tbl_book_ads_temp.work_year='".$_SESSION['work_year']."') AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR c.name LIKE '%".$name."%' OR u.mobile LIKE '%".$name."%' OR u.email LIKE '%".$name."%') ORDER BY id DESC");
			
			$config = array();
			$config["base_url"] = base_url() . "admin/generate_ro/index";
		...
		{
			$book_ads = $this->db->query("SELECT tbl_book_ads_temp.*, n.name as newspaper_name, t.name as type_name, c.name as cat_name, u.email, u.mobile FROM tbl_book_ads_temp
INNER JOIN tbl_newspapers n ON n.id=tbl_book_ads_temp.newspaper_id
INNER JOIN tbl_news_type t ON t.id=tbl_book_ads_temp.type_id
INNER JOIN tbl_categories c ON c.id=tbl_book_ads_temp.cat_id
INNER JOIN tbl_client u ON u.id=tbl_book_ads_temp.u_id WHERE tbl_book_ads_temp.pending='C' AND tbl_book_ads_temp.type_id='1' and tbl_book_ads_temp.work_year='".$_SESSION['work_year']."' order by tbl_book_ads_temp.id desc" );
			$config = array();
			$config["base_url"] = base_url(). "admin/generate_ro/index";
			$config["total_rows"] = $book_ads->num_rows();
			$config["per_page"] = 20;
...
INNER JOIN tbl_news_type t ON t.id=tbl_book_ads_temp.type_id
INNER JOIN tbl_categories c ON c.id=tbl_book_ads_temp.cat_id
INNER JOIN tbl_client u ON u.id=tbl_book_ads_temp.u_id WHERE tbl_book_ads_temp.pending='C' AND tbl_book_ads_temp.type_id='1' and tbl_book_ads_temp.work_year='".$_SESSION['work_year']."' order by tbl_book_ads_temp.id desc limit {$config['per_page']} offset {$page}");
		...
		$this->load->view('admin/generate_ro',$data);
	...
	public function get_ros()
	{
		...
		{	
			...
			//	$date_t1=date_format($date_t,"Y-m-d");			
				...
				$query = $this->db->query("SELECT tbl_book_ads_temp.*, n.name as newspaper_name, t.name as type_name, c.name as cat_name, e.name, u.email, u.mobile, u.client_name FROM tbl_book_ads_temp
				INNER JOIN tbl_newspapers n ON n.id=tbl_book_ads_temp.newspaper_id
				INNER JOIN tbl_news_type t ON t.id=tbl_book_ads_temp.type_id
				INNER JOIN tbl_categories c ON c.id=tbl_book_ads_temp.cat_id
				INNER JOIN tbl_admin e ON e.id=tbl_book_ads_temp.e_id
				INNER JOIN tbl_client u ON u.id=tbl_book_ads_temp.u_id WHERE tbl_book_ads_temp.book_id='".$client."' and pending='C' AND tbl_book_ads_temp.work_year='".$_SESSION['work_year']."' ")OR $this->db->query("SELECT tbl_book_ads_temp.*, n.name as newspaper_name, t.name as type_name,p.position as cat_name, e.name, u.email, u.mobile, u.client_name FROM tbl_book_ads_temp
				INNER JOIN tbl_newspapers n ON n.id=tbl_book_ads_temp.newspaper_id
				INNER JOIN tbl_news_type t ON t.id=tbl_book_ads_temp.type_id
				INNER JOIN tbl_position p ON p.id=tbl_book_ads_temp.cat_id
				INNER JOIN tbl_admin e ON e.id=tbl_book_ads_temp.e_id
				INNER JOIN tbl_client u ON u.id=tbl_book_ads_temp.u_id WHERE tbl_book_ads_temp.book_id='".$client."' and pending='C' AND tbl_book_ads_temp.work_year='".$_SESSION['work_year']."' ");
	    ...
	    foreach($id as $d)
	    {
	      ...
	      $this->db->query("Delete from tbl_ro_dop_temp where ro_id='$d'");
	       ...
	       $this->ro_model->update_ro_no($where);
	       	    ...
	       	    $this->session->set_flashdata('msg',"ROs released successfully");
	   $this->session->flashdata('msg');
	   redirect('admin/generate_ro/add');
	...
	public function add()
	{
        ...
        $this->db->distinct();
        $query = $this->db->get('tbl_book_ads_temp');
        //$query->result();
		//	$query = $this->db->get('tbl_book_ads_temp');
	...
	public function edit($id)
	{
			...
			//echo $data['bill']->client_id; die;
			
			$query = $this->db->query("SELECT tbl_book_ads_temp.*, n.name as newspaper_name, t.name as type_name, c.name as cat_name, e.e_name, u.email, u.mobile, u.client_name FROM tbl_book_ads_temp
			INNER JOIN tbl_newspapers n ON n.id=tbl_book_ads_temp.newspaper_id
			INNER JOIN tbl_news_type t ON t.id=tbl_book_ads_temp.type_id
			INNER JOIN tbl_categories c ON c.id=tbl_book_ads_temp.cat_id
			INNER JOIN tbl_employee e ON e.e_id=tbl_book_ads_temp.e_id
			INNER JOIN tbl_client u ON u.id=tbl_book_ads_temp.u_id WHERE tbl_book_ads_temp.u_id='".$data['bill']->client_id."' AND tbl_book_ads_temp.publish_day >'0'");
	
			$data['unbilled_ros']= $query->result();
			// echo $this->db->last_query();
			// echo '<pre>'; var_dump($data['unbilled_ros']); die;
			
			$this->db->select('tbl_bill_details.*,tbl_bill.client_id as client_id,tbl_newspapers.name as newspaper_name,tbl_categories.name as cat_name');
			$this->db->from('tbl_bill_details');
			$this->db->join('tbl_book_ads_temp', 'tbl_book_ads_temp.id = tbl_bill_details.ro_id',left);
			...
			$query = $this->db->where(["tbl_bill_details.bill_id"=>$data['bill']->id])->get();
...
//die;
	...
	{	
		if(!empty($this->input->post()))
		{
			...
			if ($this->form_validation->run() == FALSE) 
			{
				...
				$book_ads=$this->db->query("SELECT tbl_book_ads_temp.*, n.name as newspaper_name, t.name as type_name, c.name as cat_name, u.email, u.mobile, u.client_name, u.discount as client_discount FROM tbl_book_ads_temp
				INNER JOIN tbl_newspapers n ON n.id=tbl_book_ads_temp.newspaper_id
				INNER JOIN tbl_news_type t ON t.id=tbl_book_ads_temp.type_id
				INNER JOIN tbl_categories c ON c.id=tbl_book_ads_temp.cat_id
				INNER JOIN tbl_client u ON u.id=tbl_book_ads_temp.u_id WHERE tbl_book_ads_temp.id='".$id."'");
			...
			else
			{
				...
				else
				{
					...
					$this->db->update('tbl_book_ads_temp',$values, "id =".$id);	
				}
			...
			else
			{
				...
				//	echo $this->db->last_query(); 
					...
					foreach($result as $ro){
						...
						//$this->db->update('tbl_book_ads_temp',['publish_day'=>0]);
					}

					$this->db->truncate("tbl_bill_temp");
					$this->db->truncate("tbl_bill_temp_details");
				//	var_dump($value1); 
					$result=$this->db->select("publish_day")->where(["id"=>$values1['ro_id']])->get("tbl_book_ads_temp")->row();
					$this->db->where('id',$values1['ro_id']);
					$this->db->update("tbl_book_ads_temp",["publish_day"=>($result->publish_day-$values1['insertion'])]);
					$result=$this->db->select("bill_dop")->where(["ro_id"=>$values1['ro_id']])->get("tbl_ro_dop_temp")->row();
					if($result->bill_dop){
						//echo "found";
						$this->db->where('ro_id',$values1['ro_id']);
						$this->db->update("tbl_ro_dop_temp",["bill_dop"=>$result->bill_dop.", ".$values1['pub_date']]);
					}else
					{
						//echo "not found";
						$this->db->where('ro_id',$values1['ro_id']);
						$this->db->update("tbl_ro_dop_temp",["bill_dop"=>$values1['pub_date']]);
					}
					$msg="5";
					echo $msg;
					return;					
		...
		if (!empty($this->input->post())) 
		{
			...
			else
			{
				...
				else
				{
					...
					$result=$this->db->select("publish_day")->where(["id"=>$values1['ro_id']])->get("tbl_book_ads_temp")->row();
					$this->db->where('id',$values1['ro_id']);
					$this->db->update("tbl_book_ads_temp",["publish_day"=>($result->insertion-$values1['insertion'])]);
					$result=$this->db->select("bill_dop")->where(["ro_id"=>$values1['ro_id']])->get("tbl_ro_dop_temp")->row();
					if($result->bill_dop){
						//echo "found";
						$this->db->where('ro_id',$values1['ro_id']);
	...
	public function bill_print($id)
	{
		...
		INNER JOIN tbl_book_ads_temp ba ON ba.id=tbl_bill_details.ro_id
...
//		LEFT JOIN tbl_book_ads_temp ba ON ba.id=tbl_bill_details.ro_id
//		LEFT JOIN tbl_position p ON p.id=tbl_bill_details.heading
//		LEFT JOIN tbl_paper_scheme s ON s.id=tbl_bill_details.scheme
//		WHERE tbl_bill_details.bill_id='".$id."'" );
//		//LEFT JOIN tbl_premimum pr ON pr.id=tbl_bill_details.premium
