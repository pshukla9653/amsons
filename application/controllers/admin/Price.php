...
INNER JOIN tbl_categories c ON c.id=tbl_ad_price.ad_cat_id WHERE (tbl_ad_price.ad_type='1') AND (n.name LIKE '%".$name."%' OR t.name LIKE '%".$name."%' OR c.name LIKE '%".$name."%')");
			...
			$config["base_url"] = base_url() . "admin/price/index";
...
INNER JOIN tbl_categories c ON c.id=tbl_cat_with_paper.cat_id WHERE tbl_cat_with_paper.newspaper_id ='".$ap[0]->newspaper_id."'");
		...
		else
		{
			...
			$ap=$query->result();
			$data['ad_price']=$ap[0];
		    $query =$this->db->query("select DISTINCT day_id from `tbl_ad_price` WHERE `newspaper_id`='".$ap[0]->newspaper_id ."' AND `city`='".$ap[0]->city ."' AND `ad_cat_id`='".$ap[0]->ad_cat_id."'AND `ad_type`='".$ap[0]->ad_type."'");
		   //echo $this->db->last_query();
		   // die;
		    	$data['dday']=$query->result_array();
			
			
		/*	$query=$this->db->query("select * from `tbl_ad_price`");
			$adprice=$query->result();
			foreach($adprice as $ad)
			{
			$query =$this->db->query("select DISTINCT day_key from `TABLE 80` WHERE `newspaper_key`='".$ad->newspaper_id ."' AND `city_key`='".$ad->city ."' AND `heading_key`='".$ad->ad_cat_id."'AND `cat`='".$ad->ad_type."'");
	echo $this->db->last_query();
            ...
            $this->db->query("update `tbl_ad_price` set `day_id`='".$day1."' WHERE `newspaper_id`='".$ad->newspaper_id ."' AND `city`='".$ad->city ."' AND `ad_cat_id`='".$ad->ad_cat_id."'AND `ad_type`='".$ad->ad_type."'");
            echo $this->db->last_query();
...
INNER JOIN tbl_categories c ON c.id=tbl_cat_with_paper.cat_id WHERE tbl_cat_with_paper.newspaper_id ='".$paper->paper_id ."'");
	...
	{		
		$date1=date_create("2017-01-19");
		$date2=date_create("2017-01-18");
		if($date1<$date2)
		{
			echo "1";
		}
